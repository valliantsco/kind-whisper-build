...
const QuizResultView = ({ result, whatsappNumber, onReset }: QuizResultViewProps) => {
  ...
  const validate = useCallback(() => {
    ...
    if (!trimmedName) {
      ...
      else if (!trimmedName.includes(" ")) errs.name = "Inclua seu sobrenome";
    }
  ...
  const renderPrimaryCard = (model: (typeof models)[0]) => {
    ...
    const nonPriceSpecs = specs.filter(
    ...
    return (
      <motion.div
        ...
        style={{
          background: "linear-gradient(165deg, hsl(var(--primary) / 0.10) 0%, hsl(0 0% 100% / 0.03) 100%)",
      ...
      >
        ...
        <div
          ...
          style={{
            ...
            borderBottom: "1px solid hsl(var(--primary) / 0.10)",
          }}
        >
        ...
        <div className="px-3.5 py-3 space-y-2.5">
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={stagger(0.6)}>
            <h4 className="font-bold text-lg text-primary-foreground">{model.name}</h4>
            <p className="text-xs text-primary-foreground/50 mt-0.5 leading-relaxed">{model.headline}</p>
          </motion.div>
          ...
          {nonPriceSpecs.length > 0 && (
            ...
            >
              {nonPriceSpecs.slice(0, 4).map((s, idx) => (
                ...
                >
                  <div className="flex items-center gap-1.5">
                    <span className="text-primary/80 pt-[3px]">{specIcon(s.label)}</span>
                    <p className="text-[9px] uppercase tracking-[0.12em] text-primary-foreground/35 font-medium">
                      {s.label}
                    </p>
                  </div>
                  <p className="text-sm font-bold text-primary-foreground/90 leading-tight truncate">{s.value}</p>
                </div>
              ))}
            </motion.div>
          )}
          ...
          {model.whyFits && (
            <motion.div
              ...
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              transition={stagger(0.8)}
            >
              <div className="flex items-start gap-2">
                <CheckCircle2 className="w-3.5 h-3.5 mt-0.5 flex-shrink-0 text-primary/70" />
                <p className="text-[11px] text-primary-foreground/55 leading-relaxed">{model.whyFits}</p>
              </div>
            </motion.div>
          )}
          ...
          {(() => {
            ...
            return (
              <motion.div
                ...
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                transition={stagger(0.9)}
              >
                {isConsulte ? (
                  <span className="text-[11px] font-medium text-primary-foreground/50 mx-auto">
                    Consulte o valor com nosso time
                  </span>
                ) : (
                  <>
                    <span className="text-[10px] uppercase tracking-wider text-primary-foreground/40 font-medium">
                      A partir de
                    </span>
                    <span className="text-sm font-bold text-primary">{priceSpec.value}</span>
                  </>
                )}
              </motion.div>
            );
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center justify-between mt-1.5">
            {priceSpec && !priceSpec.value.toLowerCase().includes("consult") ? (
              <span className="text-xs font-bold text-primary">{priceSpec.value}</span>
            ) : (
              <span className="text-[10px] text-primary-foreground/30">Consulte</span>
            )}
            <button
              type="button"
  ...
  return (
    <motion.div key="result" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4 pb-0">
      ...
      {!hasModels && result.suggestions.length > 0 && (
        <div>
          <p className="text-[10px] font-semibold uppercase tracking-[0.15em] text-primary-foreground/30 mb-2">
            Modelos sugeridos
          </p>
          <div className="space-y-2">
            {result.suggestions.map((s) => (
              <div
                key={s}
                className="rounded-xl px-4 py-3 text-sm font-medium text-primary-foreground/70"
                style={{
      ...
      <motion.div
        ...
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={stagger(1.0)}
      >
        <div className="flex items-center gap-1">
          <ShieldCheck className="w-3 h-3 text-primary" />
          <span className="text-[9px] text-primary-foreground/35 font-medium">Garantia de fábrica</span>
        </div>
        <div className="flex items-center gap-1">
      ...
      >
        ...
        <div>
          ...
          />
          {errors.name && (
            <p className="text-[10px] mt-1 text-destructive">{errors.name}</p>
          )}
        </div>
        ...
        <div>
          ...
          />
          {errors.phone && (
            <p className="text-[10px] mt-1 text-destructive">{errors.phone}</p>
          )}
        </div>
        ...
        <motion.button
          ...
          whileHover={
            isFormValid && !isLoading
              ? { scale: 1.02, boxShadow: "0 0 25px hsl(var(--primary) / 0.5), 0 0 50px hsl(var(--primary) / 0.2)" }
              : {}
          }
        ...
        {!isFormValid && (
          <p className="text-[10px] text-primary-foreground/25 text-center">
      ...
      {hasModels && models.length > 1 && (
        <motion.div
          ...
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={stagger(1.3)}
        >
          <div
            className="h-[1px]"
            style={{ background: "linear-gradient(90deg, transparent, hsl(0 0% 100% / 0.06), transparent)" }}
          />
          <p className="text-[10px] font-semibold uppercase tracking-[0.15em] text-primary-foreground/20 pt-0">
